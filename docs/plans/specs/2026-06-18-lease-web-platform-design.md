...
## 9-1. 구현 중 확정된 추가 결정 (2026-06-18)

- **이미지**: 이번 범위는 **DB에 적재된 이미지로 표시**만. 이미지 수정·삭제용 **관리(admin) 페이지는 별도 후속 작업**으로 분리(이번 MVP 제외). 버킷은 public 전제.
- **추이 차트**: 컴포넌트는 지금 구현하되 데이터가 1개월뿐이면 "다음 달부터 추이 표시" 안내문구. 월별 적재되면 자동으로 선이 그려짐.
- **Codex 리뷰**: 각 Phase(0~4) 완료 시점마다 `codex-rescue`로 산출물 리뷰 후 다음 Phase 진행.
- **에이전트 팀**: 팀을 구성해 Phase별 병렬·협업 진행. 모르는 것/바뀌는 것은 사용자에게 즉시 질문.

### 후속 로드맵 (이번 MVP 이후)
- 이미지 관리 페이지 (업로드·교체·삭제·대표사진 지정) — 인증 필요해질 수 있음.
- CRM/영업도구 (clients·proposals) — SCHEMA.md 로드맵.
